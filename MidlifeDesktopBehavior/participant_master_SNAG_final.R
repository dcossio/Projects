...
pm_colnames <- c("Subject", paste0("tm_", DOI), paste0("ts_", DOI), paste0("em_", DOI), paste0("es_", DOI), paste0("enodes_", LETTERS), objlmarks)
...
for (i in 1:length(tm_split)) {
  ...
  explore <- tm_split[[i]][grepl("ExploreProc", tm_split[[i]]$Procedure), ]  # For each subject, grab the explore session trial data
  trials <- tm_split[[i]][grepl("TrialProc", tm_split[[i]]$Procedure), ]
  ...
  two <- tm_split[[i]]
  n_trial <- nrow(two[grepl("TrialProc", two$Procedure), ])
  n_explore <- nrow(two[grepl("ExploreProc", two$Procedure), ])
  ...
  #Had a lot of issues here. Seems i got it working so it should be fine-DC 
  for (d in 1:length(DOI)){
    trial_means <- c(trial_means, mean(trials[DOI[d]][ ,1], na.rm = TRUE))
    trial_sds <- c(trial_sds, sd(trials[DOI[d]][ ,1], na.rm = TRUE))
    explore_means <- c(explore_means, mean(explore[DOI[d]][ ,1], na.rm = TRUE))
    explore_sds <- c(explore_sds, sd(explore[DOI[d]][ ,1], na.rm = TRUE))
  }
  
 
  p_master <- c(as.character(tm_split[[i]]$Subject[1]), trial_means, trial_sds, explore_means, explore_sds, oall_node_table, obj_lmark_table)
