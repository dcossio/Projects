...
#Summary Stats
  #young male 
yng_male_stats[3,] <- summary((yng_male_analysis[,c("TotalObjVisits")])) %>% broom::tidy() %>% mutate(SD= sd(yng_male_analysis[,c("TotalObjVisits")])) %>% mutate(SEM= std.error(yng_male_analysis[,c("TotalObjVisits")])) %>% mutate(Explore_variable = "TotalObjVisits", .before= minimum) %>% mutate(Group = "young male", .before= Explore_variable) %>% mutate(N= nrow(yng_male_analysis), .after=Group)

master_stats <- rbind(master_stats,yng_male_stats[3,])

  # Young female
yng_female_stats[3,] <- summary((yng_female_analysis[,c("TotalObjVisits")])) %>% broom::tidy() %>% mutate(SD= sd(yng_female_analysis[,c("TotalObjVisits")])) %>% mutate(SEM= std.error(yng_female_analysis[,c("TotalObjVisits")])) %>% mutate(Explore_variable = "TotalObjVisits", .before= minimum) %>% mutate(Group = "young female", .before= Explore_variable)%>% mutate(N= nrow(yng_female_analysis), .after=Group)

master_stats <- rbind(master_stats,yng_female_stats[3,])

# Midlife male 
mid_male_stats[3,] <- summary((mid_male_analysis[,c("TotalObjVisits")])) %>% broom::tidy() %>% mutate(SD= sd(mid_male_analysis[,c("TotalObjVisits")])) %>% mutate(SEM= std.error(mid_male_analysis[,c("TotalObjVisits")])) %>% mutate(Explore_variable = "TotalObjVisits", .before= minimum) %>% mutate(Group = "midlife male", .before= Explore_variable)%>% mutate(N= nrow(mid_male_analysis), .after=Group)

master_stats <- rbind(master_stats,mid_male_stats[3,])

#midlife female
mid_female_stats[3,] <- summary((mid_female_analysis[,c("TotalObjVisits")])) %>% broom::tidy() %>% mutate(SD= sd(mid_female_analysis[,c("TotalObjVisits")])) %>% mutate(SEM= std.error(mid_female_analysis[,c("TotalObjVisits")])) %>% mutate(Explore_variable = "TotalObjVisits", .before= minimum) %>% mutate(Group = "midlife female", .before= Explore_variable)%>% mutate(N= nrow(mid_female_analysis), .after=Group)

master_stats <- rbind(master_stats,mid_female_stats[3,])
