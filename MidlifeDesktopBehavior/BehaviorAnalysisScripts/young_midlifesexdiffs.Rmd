...
No sex differencecs in midlife. sex differences in young
```{r accuracy}
# only young male and midlife female 
#Proportion Correct (Spatial Memory)
female_mid_acc<-mid_master_participant %>% filter(sex== "Female") %>% select(tm_accuracy)

#Proportion Correct (Spatial Memory)
male_mid_acc<-mid_master_participant%>% filter(sex== "Male") %>% select(tm_accuracy)
#young 
female_yng_acc<-yng_master_participant %>% filter(sex== "Female") %>% select(tm_accuracy)
#Proportion Correct (Spatial Memory)
male_yng_acc<-yng_master_participant%>% filter(sex== "Male") %>% select(tm_accuracy)



#adding to master analysis df
...
shapiro.test(female_mid_acc$tm_accuracy)
shapiro.test(male_mid_acc$tm_accuracy)
shapiro.test(female_yng_acc$tm_accuracy)
shapiro.test(male_yng_acc$tm_accuracy)


#Wilcoxon Test 
propcorr_groupdiff<-wilcox.test(female_yng_acc$tm_accuracy,male_yng_acc$tm_accuracy)
propcorr_groupdiff_effsize<-cohensD(female_yng_acc$tm_accuracy,male_yng_acc$tm_accuracy)


propcorr_groupdiff<-wilcox.test(female_mid_acc$tm_accuracy,male_mid_acc$tm_accuracy)
propcorr_groupdiff_effsize<-cohensD(female_mid_acc$tm_accuracy,male_mid_acc$tm_accuracy)
...
mid_big_explore_counts <- mid_big_explore_counts %>% mutate(sex=mid_explr_Subinfo$sex) %>% mutate(age =mid_explr_Subinfo$age)
...
yng_big_explore_counts <- yng_big_explore_counts %>% mutate(sex=yng_explr_Subinfo$sex) %>% mutate(age =yng_explr_Subinfo$age)
...
# no major  sex effect midlife female and young male only
```{r Pause Duration}
# Young 
