...
summary(dist_aov)
# sex is associated with significant different distances 
# age is significantly associated with difference in distance lengths 
# there's no sex vs age interaction

# Post hoc
with(dist, pairwise.t.test(Dist, group, 
p.adj="bonferroni", paired=F))



 # 2 is to create our factors of midlife men /midlife women /young men/young women
# Does sex affect distance and does this change with age?
d_aov <- aov(formula=Dist ~ group, dist)
summary(d_aov)

# Post hoc 
 TukeyHSD(d_aov, which = "group")
 

 # we dont find any meaning ful significant interactions
 # only age effect in females and a yng male vs young female
...
o_aov <- aov(formula=TotalObjVisits ~ group, ObjVisits)
summary(o_aov)
