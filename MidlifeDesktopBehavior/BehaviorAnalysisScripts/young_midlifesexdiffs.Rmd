...
```{r accuracy}
# create a dataframe with all subjects 

# grab all midlife 
AccDF <- mid_master_participant %>% select(Subject,sex,age,tm_accuracy)
# grab all yng
AccDF <- rbind(AccDF, yng_master_participant[,c("Subject","sex","age","tm_accuracy")])

# normality test 
# Midlife Female 
AccDF %>% filter(sex == "Female" &
                   age == "midlife") %>%  select(tm_accuracy) %>% {
                     shapiro.test(.$tm_accuracy)
                   }
#Midlife male 
AccDF %>% filter(sex == "Male" &
                   age == "midlife") %>%  select(tm_accuracy) %>% {
                     shapiro.test(.$tm_accuracy)
                   }
#yng female 
AccDF %>% filter(sex == "Female" &
                   age == "young") %>%  select(tm_accuracy) %>% {
                     shapiro.test(.$tm_accuracy)
                   }
# Young males 
AccDF %>% filter(sex == "Male" &
...
acc_aov <-  aov(formula= tm_accuracy ~ sex * age, AccDF)
summary(acc_aov)

# post hoc non parametric tests
# #Wilcoxon Test midlife 
wilcox.test(AccDF[AccDF$sex == 'Male' & AccDF$age == "midlife", c("tm_accuracy")],AccDF[AccDF$sex == 'Female' & AccDF$age == "midlife", c("tm_accuracy")])
#young wilcox ttest 
...
mid_female_analysis<-mid_master_participant %>% filter(sex == "Female") %>% select(Subject,sex,age) %>% as.data.frame()      #creating master analysis data-frame to store all final exploration variable outputs into 

mid_male_analysis<-mid_master_participant %>% filter(sex == "Male") %>% select(Subject,sex,age) %>% as.data.frame() 
  
yng_female_analysis<-yng_master_participant %>% filter(sex == "Female") %>% select(Subject,sex,age) %>% as.data.frame()

yng_male_analysis<-yng_master_participant %>% filter(sex == "Male") %>% select(Subject,sex,age) %>% as.data.frame()  #same thing for young
...
halls <- rbind(halls,yng_female_analysis) %>% select(Subject,sex,age,TotalHallwayVisits)
...
Turns <- rbind(Turns,mid_female_analysis) %>% select(Subject,sex,age,turns)
#Normality test to assess whether samples are normally distributed 
...
Pauses <- rbind(Pauses,mid_female_analysis) %>% select(Subject,sex,age,pauses)
...
Spread <- rbind(Spread,mid_female_analysis) %>% select(Subject,sex,age,evenness)
...
#Binding the mid male and female evenness spread into one data frame for cleaner analysis 
Hallspread <- rbind(Hallspread,mid_female_analysis) %>% select(Subject,sex,age,HallEvenness)
...
BP <- rbind(BP, mid_female_analysis)%>% select(Subject,sex,age,ButtonPress)
...
A normality test was run on each group. The data was not normally distributed for most groups: Midlife males(p-value = 0.01381), Midlife Females (p-value = 0.000541), Young males (p-value =  0.0001843), and young females (p-value = 0.3501). A two factor anova revealed an effect of sex (0.000274) and an effect of age (< 2e-16)
```{r Longest Hallway}
