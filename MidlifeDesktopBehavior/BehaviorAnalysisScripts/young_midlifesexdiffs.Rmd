...
dist %>% 
  aov_ez(id = 'Subject', dv = 'Dist', within = 'group' , covariate = list('age', 'sex'))


mod <- dist %>% 
  mutate(sub = as.factor(Subject), sex = scale(sex), age = scale(age)) %>%
...
shapiro.test(mid_male_analysis$TotalObjVisits)
shapiro.test(mid_female_analysis$TotalObjVisits)
