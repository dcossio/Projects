...
yngsam_size <- ynginfo %>% group_by(Sex)%>% summarize(num=n())
View(youngAccDF)
...
shapiro.test(yngfemaleAcc$AvgAccuracy)  # non normal

yngmaleAcc <- youngAccDF %>% filter(Sex == "Male")
shapiro.test(yngmaleAcc$AvgAccuracy)  # non normal

# young men vs women 
wilcox.test(yngfemaleAcc$AvgAccuracy,yngmaleAcc$AvgAccuracy)

# young women vs midlife women 
