...
# <span style="color: pink;"> ROI based fiber tracking  </span>  {.tabset .tabset-dropdown}
```{r, The code below is required to combine all of the subject csvs into one spot ,echo=FALSE, results='hide',message=FALSE}
...
ROI_based_tract_DF <- read.csv("/Users/danielacossio/Documents/Chrastil_Lab/Projects/28andB/FInalizing_manuscript_072024/Re_extraction_2024_06_27/ROI_tractometry/QA_master_DF.csv")
...
ROIS <-colnames(ROI_based_tract_DF[,3:ncol(ROI_based_tract_DF)])
...
# <span style="color: pink;"> Automatic tracking based fiber tracking  </span>  {.tabset .tabset-dropdown}
```{r, The code below is required to combine all of the subject csvs into one spot}
...
tract_workdir <-paste0(Main_workdir,"Re_extraction_2024_06_27/Automatic_tracking/")
...
#ROIS_cols <- length(c("Arcuate_Fasciculus_L","Corpus_Callosum","Inferior_Fronto_Occipital_Fasciculus_R","Inferior_Longitudinal_Fasciculus_L"))
#The ROI ccode below is if we want every single one of the rois in one csv. For my sake i'm only grabbing a few specific ones
ROIS_cols <- length(list.dirs(paste0(tract_workdir,subfiles[sub]),full.names = FALSE,recursive = FALSE)) # Changing the ROI

Automatic_based_tract_DF <- as.data.frame(matrix(ncol = ROIS_cols+1, nrow = length(subfiles)))
...
for(sub in 1:length(subfiles)){
  ...
  ROIS <- list.dirs(paste0(tract_workdir, subfiles[sub]),full.names = FALSE,recursive = FALSE)
  ...
  print(paste("running ",ROIS[roi]))
  text_file <- list.files(paste0(tract_workdir,subfiles[sub],"/",ROIS[roi]),pattern="*.txt")
  
  text_file_path <- paste0(tract_workdir,subfiles[sub],"/",ROIS[roi],"/",text_file)
  
 
      # Checking that the text file is not empty.
      if (file.size(text_file_path) > 0 ){
        print("File Exists lets read it in")
        #ROI_txt <- read.delim(paste0(tract_workdir,subfiles[sub],"/",ROIS[roi],"/",text_file), header=FALSE) %>% filter(V1 %in% c("qa"))
        ROI_txt <- read.delim(text_file_path, header = FALSE) %>% filter(V1 %in% c("qa"))
  ...
  Automatic_based_tract_DF[sub,] <- temp_df[1,]

}


#
colnames(Automatic_based_tract_DF) <- c("subjects",ROIS)
# colnames(MD_master_DF) <- c("subjects",ROIS$V1)
#
# QA_master_DF <- QA_master_DF %>% filter(.$subjects != "boxplots")
#
...
# Let's get our entire QA ROI Data frame
Automatic_based_tract_DF <- Automatic_based_tract_DF %>% filter(!(subjects %in% c("sub-27", "sub-07"))) %>% mutate(trimester=AllScanInfo$trimester)

Automatic_based_tract_DF <- master_DF %>%  mutate(trimester=AllScanInfo$trimester)

ROIS <-c ("Inferior_Fronto_Occipital_Fasciculus_R", "Inferior_Longitudinal_Fasciculus_L", "Arcuate_Fasciculus_L")
#,")
...
for (ROI in 1:length(ROIS)) {
  ...
  y <- ROIS[ROI]
#
  QAplot <- Automatic_based_tract_DF %>%
    ggplot(aes(x = trimester, y = Automatic_based_tract_DF[,ROIS[ROI]], fill = trimester)) +
    #geom_jitter(color = "black",size = 0.8, alpha = 0.9,width = 0.25) +
    geom_boxplot(linewidth=0.5) +
    theme_bw() + scale_fill_manual(values = grp_colors)+
    # scale_y_binned(n.breaks=5, nice.breaks = TRUE) +
