...
##### **Participants**
  ...
  * 23 scans were preprocessed successfully. 2 scans removed due to issues with the DWI.
...
For all figures please see the google folder below.
https://drive.google.com/drive/folders/1-V_XaS87D_WtO-IjLsjRZdveZ0iu1Hzr?usp=sharing
...
Main_workdir <-"/Users/danielacossio/Downloads/Global_QA/"
tract_workdir <-"/Users/danielacossio/Downloads/Global_QA/tractometry/"
...
# ROIS_cols <- length(list.dirs(paste0(tract_workdir,subfiles[1])))
...
#   text_file <- list.files(path = ROIS[roi], pattern = "*.txt")
#     # Check if CSV exists
#     
...
#       if (file.size(paste0(ROIS[roi],"/",text_file)) > 0 ){
#         print("File Exists lets read it in")
#         ROI_txt <- read.delim(paste0(ROIS[roi],"/",text_file), header=FALSE) %>% filter(V1 %in% c("qa"))
...
# reading in info 
AllScanInfo <- read.csv(paste0(Main_workdir,"allscansInfo.csv"), header=TRUE) %>% `colnames<-`(c("scans","gest_week","Trimester")) %>%  filter(!(scans %in% c("7", "27")))
AllScanInfo$scans <- lapply(AllScanInfo$scans, sprintf, fmt = "%03d")
AllScanInfo <- ChangeTrimesterLabels(AllScanInfo)
AllScanInfo$gest_week <- as.numeric(AllScanInfo$gest_week)

# Let's get our entire QA ROI Data frame 
QA_ROI_master_DF <- read.csv(paste0(Main_workdir,"ROI_QA_master_DF.csv")) %>% filter(!(subjects %in% c("sub-007", "sub-027"))) %>% mutate(trimester = factor(AllScanInfo$Trimester, levels= c('Pre','1st','2nd', '3rd', 'Post')))

ROIS <-c ("Arcuate_Fasciculus_L","Corpus_Callosum", "Inferior_Longitudinal_Fasciculus_L", "Cortico_Striatal_Pathway_L")
...
#
  QAplot <- QA_ROI_master_DF %>%
    ...
    pdf(file = paste0(Main_workdir,y,"_QA.pdf"),width=10, height=8)
    print(QAplot)
    dev.off()
}
...
for (ROI in 2:nrow(ROIS)) {
  ...
  MDplot <- MD_scans %>%
    ggplot(aes(x = trimester, y = MD_scans[, ROI], group = trimester)) +  
    #geom_jitter(color = "black",size = 0.8, alpha = 0.9,width = 0.25) +
   ...
   #
   pdf(file = paste0(tract_workdir,"/boxplots/MD/",y,"_MD.pdf"),width=10, height=8)
   print(MDplot)
   dev.off()
}
...
pdf(file = "/Users/danielacossio/Downloads/WholeBrain_GAM.pdf",
    width = 10,
    height = 5)
