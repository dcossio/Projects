...
<p style="text-align:center" > <strong>Partial Correlations</strong> </p>

Here we want to know whether ROIs correlate to our different behavioral variables when we control for age and sex. So we will run partial correlations.When controlling for age and sex, our ROIS are not significantly correlated
<br>
...
<p style="text-align:center" > <strong>Multiple Linear Regression</strong> </p> 
<p style="text-align:left " > formula used is accuracy ~ ROI_FA + Sex + age_scan_years. Age appears to be the larger factor here not sex or even ROI in predicting maze accuracy behavior.</p>
...
TableOutput(FA_DSP_male_results, "none", 400, 200)
```

For women, nothing was significantyl correlated 
```{r, echo=FALSE,results='hide', message=FALSE }
FA_DSP_female_results <- AprioriPcorTest(dataframe = female_apriori_FA, task ="dsp_si_true_pct",covariates = c("age_scan_years"))
TableOutput(FA_DSP_female_results, "none", 400, 200)
```
