...
RunningPcorTest <- function(dataframe, task, covariates){
  ...
  for (roi in c(ROIs$V2)){
    ...
    pcortestresults <-pcor.test(dataframe[[roi]],dataframe[[task]], dataframe[, c(covariates)], method = "spearman")
...
```{r, echo=FALSE, message=FALSE }
TableOutput(AprioriPcorTest(dataframe = male_apriori_FA, task ="maze_accuracy_pct",covariates = c("age_scan_years")), "classic", 400, 200)
```
<br>
...
```{r, echo=FALSE, message=FALSE }
TableOutput(AprioriPcorTest(dataframe = female_apriori_FA, task ="maze_accuracy_pct",covariates = c("age_scan_years")), "classic", 400, 200)
```
...
`r TableOutput(finalDF[["Medial_lemniscus_R"]], "classic", 500,150) `
<br>

<p style= "text-align: center;"> SFOF</p> 
`r TableOutput(finalDF[["Superior_fronto-occipital_fasciculus_(could_be_a_part_of_anterior_internal_capsule)_R"]], "classic", 500,150) `
`r TableOutput(finalDF[["Superior_fronto-occipital_fasciculus_(could_be_a_part_of_anterior_internal_capsule)_L"]], "classic", 500,150) `
<br>
