...
```{r, summary stats for behavior by sex }
...
```{r, summary stats for scan + behavior by sex }

DSP_DSI_DF %>% dplyr::summarise(count = n(), mean = mean(age_spatial_years, na.rm = TRUE), sd = sd(age_spatial_years, na.rm = TRUE), min(age_spatial_years, na.rm = TRUE), max(age_spatial_years, na.rm = TRUE))

```

```{r, summary stats for + behavior by sex }

group_by(DSP_DSI_DF, sex) %>% dplyr::summarise(count = n(), mean = mean(age_spatial_years, na.rm = TRUE), sd = sd(age_spatial_years, na.rm = TRUE), min(age_spatial_years, na.rm = TRUE), max(age_spatial_years, na.rm = TRUE))
...
```{r, estradiol comparison boxplot}
...
```{r, Progesterine comparison boxplot}
...
# Step 1: Normality check and visualization of data  {.tabset .tabset-pills}
...
```
**Two-tailed correlation NOT controlling for age**
This is not what we want but i'm leaving it in here in case i forget that i already did this. 
```{r, warning=FALSE, "2 tailed Est and SI correlation controlling age using Behavioronly_data_dsp"}
cor.test(Behavioronly_Female_data_dsp$estradiol_spatial_pg_ml,
          Behavioronly_Female_data_dsp$dsp_si_true_pct,
          method = "spearman") #p-value=0.003
...
**Two-tailed correlation controlling for AGE using all females**
This is not what we want but i'm leaving it in here in case i forget that i already did this. 
```{r, warning=FALSE, "2 tailed fsh and SI correlation controlling age using "}

#pcor doesn't like missing values so 
#let'sm remove na

ppcor::pcor.test(FSH__Female_data_dsp$fsh_spatial_miu_ml,FSH__Female_data_dsp$dsp_si_true_pct, FSH__Female_data_dsp$age_spatial_years, method = "spearman") #p-value=0.414
...
. 
```{r, warning=FALSE, "2 tailed Est and SI correlation controlling age using Behavioronly_data_dsp"}
cor.test(Behavioronly_Female_data_dsp$progesterone_spatial_ng_ml,
          Behavioronly_Female_data_dsp$dsp_si_true_pct,
          method = "spearman") #p-value=0.05
...
male_data_dsp <- Behavioronly_data_dsp %>%  filter(sex == "Male") %>% filter(!is.na(testosterone_spatial_ng_dl))

# this one uses spatial hormones
cor.test(male_data_dsp$testosterone_spatial_ng_dl,
         male_data_dsp$dsp_si_true_pct,
         conf.level = 0.95,
         method = "spearman")
...
# this one uses spatial hormones
cor.test(Behavioronly_Female_data_dsp$testosterone_spatial_ng_dl,
         Behavioronly_Female_data_dsp$dsp_si_true_pct,
         conf.level = 0.95,
         method = "spearman")
...
DSP_ANOVA <- kruskal.test(dsp_si_true_pct ~ repo_status, data = DSP_DSI_DF)
#There is a marginal effect of REPO on performance 

dunn.test::dunn.test(DSP_DSI_DF$dsp_si_true_pct, g=DSP_DSI_DF$repo_status, method = "bh")
...
```{r, post hoc }
wilcox.test(df$estradiol_spatial_pg_ml, df_DSP_peri_post$estradiol_spatial_pg_ml) %>% broom::tidy() %>%  mutate(hormone = "Estradiol") %>% dplyr::select("hormone",everything())
...
# 9 x 6 
DSP_DSI_DF %>% ggplot(aes(x = factor(repo_status, levels=c("Men", "Pre", "Peri_Post")), y = dsp_si_true_pct, fill= repo_status)) +
  geom_boxplot(width=0.5,fatten = 1, show.legend = TRUE) +
  geom_point(shape=16) +
  scale_fill_manual(values=c("#d3d0c6","#7d4c5a","#CD8A9B"))+  #"#414143")
  scale_y_continuous(breaks = seq(0, 1, by = 0.2)) +
   xlab("Sex")+
   ylab("Solution Index") +
  theme( axis.title.x=element_blank(),
...
```{r, fnamegroup boxplot}

# 9 x 6 
DSP_DSI_DF %>% ggplot(aes(x = factor(repo_status, levels=c("Men", "Pre", "Peri", "Post")), y = FNAME, fill= repo_status)) +
  geom_boxplot(width=0.5,fatten = 1, show.legend = TRUE) +
  geom_point(shape=16) +
  scale_fill_manual(values=c("#d3d0c6","#CD8A9B", "#7d4c5a","#414143"))+
  scale_y_continuous(breaks = seq(0, 1, by = 0.2)) +
   xlab("Sex")+
   ylab("FNAME") +
  theme(axis.title.x = element_text(vjust=- 4, size = 16),
