...
These were excluded later from hippocampal analyses so i just exlcuded their behavior here as well. Adding these in gives me 77 which is still not the correct number of subjects. I'm not sure which one is the extra. For now I'll add them back in but it shouldn't affect much. 
...
She also did a partial cor to control for age. here it is still significant even when controlling for age.
...
Here, its another one tailed correlation test 
```{r, warning=FALSE}
...
Step 1 of mediation is to complete the first correlation in which X= fornix and Y = DSP. WE need to regress our x and y. p=0.03803 
...
Step 2 of mediation is to complete the next correlation in which M= hormones and X = Fornix. p=0.9646 So here the MD  and estradiol are not significant. 
...
Step 1 of mediation is to complete the first correlation in which X= MD whole fornix and Y = DSP. WE need to regress our x and y, p=0.03803
