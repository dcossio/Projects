...
avgCA23_plot <- ggplot(df,aes(y=loop_ae_avg_degree,x=t2hipp_vol_avg_ca23,color=sex)) + theme_bw() + geom_point() + geom_smooth(method = "lm", aes(color = factor(sex))) + stat_cor(aes(color= sex),method = "spearman", label.x.npc = 0.025, label.y.npc = 0.75) + scale_colour_manual(values = c(colors[4],colors[5]))
...
avgCA1_dt_plot <- ggPredict(avgCA1_dt_fit,terms ="t2hipp_vol_avg_ca1", se=TRUE,interactive=FALSE)
...
```{r, Avg CA23 + AVG Degrees Traveled MLR, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}

avgCA23_dt_fit <- lm(loop_de_avg_degree ~ t2hipp_vol_avg_ca23 + age_spatial_years + sex ,data= df )

summary(avgCA23_dt_fit)

avgCA23_dt_plot <- ggPredict(avgCA23_dt_fit,terms ="t2hipp_vol_avg_ca23", se=TRUE,interactive=FALSE)
