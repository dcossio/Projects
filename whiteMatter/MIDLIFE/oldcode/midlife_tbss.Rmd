...
# only select specific ROIs suhc as fornix and cingulum 
rawAD <- read_csv("JHU-skeletonized-AD.csv") 

rawRD <- read_csv("JHU-skeletonized-RD.csv")

rawMD<- read_csv("JHU-skeletonized-MD.csv")

rawFA<- read_csv("JHU-skeletonized-FA.csv") %>% select(Fornix,`Fornix (cres) / Stria terminalis R, Fornix (cres) / Stria terminalis L,Cingulum (cingulate gyrus) R,Cingulum (cingulate gyrus) L,Cingulum (hippocampus) R, Cingulum (hippocampus) L)
