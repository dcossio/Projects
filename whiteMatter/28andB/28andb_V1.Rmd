...
Est_QA_all_pos_values_long <- gather(Est_QA_all_pos_values, ROI, QA ,QA_est_gathercols, factor_key = TRUE) %>% select(!(Subjects))
...
## GAther it long for large compiled figure
Est_MD_all_neg_values_long <- gather(Est_MD_all_neg_values, ROI, MD ,MD_est_gathercols, factor_key = TRUE) %>% select(!(Subjects))
```
...
prog_QA_cols <- c("Subjects", as.character(prog_QA_all_pos[,1]))
...
QA_prog_gathercols <- colnames(prog_QA_all_pos_values[5:22])
...
<span style="color: hotpink;"> Positive ( FDR= 0.005232 ) </span> 
```{r, layout="l-body-outset",echo=FALSE}
...
<span class="text-center" style="color: Teal;"> Neg ( FDR= 0.024925)</span> 
```{r, layout="l-body-outset",echo=FALSE}
...
```{r Looping through all QA progesterone and making plots Specific variables}
...
for (ROI in QA_prog_gathercols) {
  ...
  y <- prog_QA_all_pos_values %>%
   ...
   png(file = paste0(prog_figpath,ROI, "pos.png"))
   print(y)
   dev.off()
}
...
```
#### All weeks 
<span style="color: hotpink;"> Positive ( FDR= 0.030986 ) </span> 
```{r, layout="l-body-outset",echo=FALSE}
...
<span class="text-center" style="color: Teal;"> Neg ( FDR= 0.069523)</span> 
```{r, layout="l-body-outset",echo=FALSE}
...
```{r Looping through all MD progesterone and making plots Specific variables}
...
for (ROI in QA_prog_gathercols) {
  ...
  y <- prog_QA_all_pos_values %>%
   ...
   png(file = paste0(prog_figpath,ROI,".png"))
   print(y)
   dev.off()
}
...
## GAther it long for large compiled figure
gest_MD_pos_values_long <- gather(gest_MD_pos_values, ROI, MD ,md_gest_pos_gathercols, factor_key = TRUE) %>% select(!(Subjects))
...
```{r, GEstational MD positive, echo=FALSE}
...
```{r, GEstational MD negative, echo=FALSE}
gest_md_figpath <- "/Users/danielacossio/Library/CloudStorage/GoogleDrive-dcossio1@uci.edu/My Drive/28andB/GestationalWks/MD/Pre_Preg_wks/"
