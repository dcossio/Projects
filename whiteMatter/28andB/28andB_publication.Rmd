...
```{r QA,echo=FALSE, echo=FALSE, results='hide',message=FALSE}
...
<!-- ## <span style="color: pink;"> Tractometry </span>  -->

<!-- ```{r Data cleaning  tractometry, echo=FALSE, results='hide',message=FALSE} -->
...
MergedWholebrain <- read.csv("/Users/danielacossio/Downloads/Global_QA/QA_master_DF.csv") %>%  mutate(Trimester = factor(AllScanInfo$Trimester, levels= c('Pre','1st','2nd', '3rd', 'Post'))) %>% mutate(gestWK = AllScanInfo$gest_week)
...
# 
# MergedWholebrain %>% ggplot(aes(x=Trimester, y=iso, fill=Trimester)) +
#     geom_boxplot() +
#     theme_bw() + scale_fill_manual(values = grp_colors)+
#     theme(
#       strip.text.x = element_text(
