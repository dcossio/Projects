...
ggplot(data = CFPR, aes(x = Cingulum_Frontal_Parietal_R, y = `Mean.Acc`)) +
  ...
  labs(title = "Correlation between proportion correct and QA in the Cingulum_Frontal_Parietal_R",
...
ggplot(data = CFPR, aes(x = Cingulum_Frontal_Parietal_R, y= Mean.Acc)) +
  ...
  stat_cor(aes(color= sex),method = "spearman", label.x.npc = 0.75, label.y.npc = 0.75) +
  xlim(min(CFPR$Cingulum_Frontal_Parietal_R), max(CFPR$Cingulum_Frontal_Parietal_R)) +
  scale_x_continuous(breaks=seq(0,4,0.5)) +
  theme(plot.title = element_text(hjust = 0.5, vjust = +2))+
  theme(axis.title.x = element_text(vjust = -2))  +
...
ggplot(data = pe_CFPR, aes(x = Cingulum_Frontal_Parietal_R, y = path_eff)) +
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  theme(legend.position = "right") +
  annotate("text",x = 1.6, y = 1.26, size = 4, colour = b[3], label=paste0("R="," ",PE_coef, ", ","P=0.2755"))+
  annotate("text",x = 2.5, y = 1.26, size = 4, colour = b[5], label=paste0("R="," ",m_coeff, ", ","P=0.3204"))+
  xlim(min(CFPR$Cingulum_Frontal_Parietal_R), max(CFPR$Cingulum_Frontal_Parietal_R)) +
  scale_x_continuous(breaks=seq(0,4,0.5)) +
  theme(plot.title = element_text(hjust = 0.5, vjust = +2))+
  theme(axis.title.x = element_text(vjust = -2))  +
...
```{r fornix in women only }

pe_CFPR <- PE_negRD %>% select(SUBJ..,path_eff,sex,Cingulum_Frontal_Parietal_R) %>% filter(sex==0)
...
PE_RDmod <- lm(pe_CFPR$path_eff~ pe_CFPR$Cingulum_Frontal_Parietal_R )
...
ggplot(data = pe_CFPR, aes(x = Cingulum_Frontal_Parietal_R, y = path_eff)) +
  ...
  geom_abline(slope=coef(PE_RDmod)["pe_CFPR$Cingulum_Frontal_Parietal_R"],intercept=coef(PE_RDmod)["(Intercept)"],colour=b[3])+
